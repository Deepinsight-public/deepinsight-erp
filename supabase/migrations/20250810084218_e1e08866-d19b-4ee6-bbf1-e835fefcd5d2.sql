-- Create compatibility views for old modules using existing table structure

-- 1. Inventory compatibility view (using existing inventory table)
CREATE OR REPLACE VIEW vw_inventory AS
SELECT 
  i.id,
  p.sku,
  p.sku as epc,  -- mapping to existing field
  p.sku as serial_no,  -- placeholder
  ...
  p.kw_code,
  p.map_price,
  'A'::text as grade_label,  -- placeholder
  i.created_at,
  i.updated_at
FROM inventory i
LEFT JOIN products p ON i.product_id = p.id;

-- 2. Warranty compatibility view from warranty tables
...
SELECT 
  ...
  0::numeric as warranty_amount,  -- placeholder
  0::integer as ext_warranty_months,  -- placeholder
  ''::text as warranty_card_no,  -- placeholder
  wh.invoice_date as warranty_start_at,
  wh.warranty_expiry as warranty_end_at,
  ''::text as customer_first_name,  -- placeholder
  ''::text as customer_last_name,  -- placeholder
  s.store_name,
...
-- 3. Unified returns view combining both return tables
...
SELECT 
  ...
  r.status,
  false as is_customer_return,  -- placeholder
  null::uuid as return_wh_id,  -- placeholder
...
-- 4. Sales orders list view with extended fields
...
SELECT 
  ...
  so.store_id,
  null::uuid as customer_id,  -- placeholder
  so.customer_first as customer_first_name,
  so.customer_last as customer_last_name,
  ''::text as cashier_first_name,  -- placeholder
  ''::text as cashier_last_name,   -- placeholder
  so.customer_source,
  so.walk_in_delivery,
  null::timestamp as delivery_date,  -- placeholder
  0::numeric as delivery_fee,  -- placeholder
  0::numeric as accessory_fee,  -- placeholder
  so.other_fee,
  so.payment_method as payment_method1,
  null::numeric as payment_amount1,  -- placeholder
  ''::text as payment_method2,  -- placeholder
  null::numeric as payment_amount2,  -- placeholder
  ''::text as payment_method3,  -- placeholder
  null::numeric as payment_amount3,  -- placeholder
  so.tax_amount / NULLIF(so.total_amount, 0) as tax_rate,
  0::numeric as deposit,  -- placeholder
  so.order_date,
  so.created_at,
  so.updated_at,
  s.store_name,
  ...
  so.customer_email,
  so.customer_phone,
  -- Aggregate calculations from order items
  so.total_amount,
  ...
  1::numeric as avg_price_map_rate,  -- placeholder
  0::numeric as total_gross_profit   -- placeholder
FROM sales_orders so
LEFT JOIN stores s ON so.store_id = s.id
LEFT JOIN sales_order_items soi ON so.id = soi.sales_order_id
GROUP BY so.id, s.store_name;

-- 5. Sales summary view for pivot analysis
...
SELECT 
  ...
  0::numeric as total_profit,  -- placeholder
  1::numeric as avg_price_map_rate  -- placeholder
FROM sales_orders so
LEFT JOIN stores s ON so.store_id = s.id
LEFT JOIN sales_order_items soi ON so.id = soi.sales_order_id
