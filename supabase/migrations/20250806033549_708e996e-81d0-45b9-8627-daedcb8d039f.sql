-- Create function to handle sales order creation with stock deduction
CREATE OR REPLACE FUNCTION create_sales_order_with_stock_deduction(
  order_data jsonb,
  line_items jsonb[]
) RETURNS TABLE(
  id uuid,
...
BEGIN
  ...
  FOR i IN 1..array_length(line_items, 1) LOOP
    ...
    WHERE product_id = (line_item->>'product_id')::uuid
      AND store_id = (order_data->>'store_id')::uuid;
